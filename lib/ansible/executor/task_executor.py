...
class TaskExecutor:
    ...
    def _execute(self, variables=None, from_loop=False):
        ...
        for attempt in xrange(1, retries + 1):
            ...
            if register:
                vars_copy[register] = result = wrap_var(result)
            vars_copy[implicit_register] = result = wrap_var(result)
            if not from_loop:
        ...
        if register:
            variables[register] = result = wrap_var(result)
        variables[implicit_register] = result = wrap_var(result)
        if not from_loop:
